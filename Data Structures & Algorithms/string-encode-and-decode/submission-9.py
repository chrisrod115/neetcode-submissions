class Solution:
    ...
    def decode(self, s: str) -> List[str]:
        ...
        while i < len(s):
            ...
            # if i want the next *count* characters after j, i look at j + 1 until j + 1 + count
            # res.append(s[j+1:j+1+count])
            # i = j + 1 + count 
            # j = i

            # neet's way is to update the j pointer beforehand (it's the same thing)
            i = j + 1
            j = i + count
            res.append(s[i:j])
            i = j
        ...
        # so for the space and time complexity? -->
        # Time: O(n)
        # Space: O(1)
        #   You only use pointers i and j, and res is directly returned (does not count as additional space as a result)